#!/usr/bin/env python3
"""
Analysis helper for .npz archives produced by simulate.py.

For each archive, the script loads time series, prints a summary, detects
revivals, and optionally exports a CSV and/or a figure.
"""
...
def print_summary(summary: Dict[str, object]) -> None:
    ...
    print(f"\n=== {summary['path']} ===")
    if grid is not None:
        print(f"Grid: {tuple(grid)} | iterations: {summary['steps']}")
    else:
        print(f"Iterations: {summary['steps']}")
    print(f"Max fidelity: {summary['max_fidelity']:.6f}")
    if summary.get("alphas_mean") is not None:
        mean = summary["alphas_mean"]
        std = summary["alphas_std"]
        print(f"mean(alpha_n): {mean:.6g} ± {std:.3g}")
...
def plot_dataset(
    ...
    ax_idx = 0
    ax = axes[ax_idx]
    ax.plot(steps, fidelities, color="tab:blue", label="Fidelity")
...
def main(argv: Optional[Iterable[str]] = None) -> None:
    parser = argparse.ArgumentParser(
        description="Analysis of .npz results generated by simulate.py.",
    )
    parser.add_argument("paths", nargs="+", type=Path, help=".npz archives to analyze")
    parser.add_argument(
        "--revival-threshold",
        type=float,
        default=0.95,
        help="fidelity threshold to detect revivals (default 0.95)",
    )
    parser.add_argument(
        "--revival-gap",
        type=int,
        default=1,
        help="minimal separation between revivals (default 1)",
    )
    parser.add_argument(
        "--csv-dir",
        type=Path,
        help="output directory to export a CSV per file (optional)",
    )
    parser.add_argument(
        "--plot",
        action="store_true",
    ...
    parser.add_argument(
        "--plot-dir",
        type=Path,
        help="output directory to save plots (PNG) without display",
    )
